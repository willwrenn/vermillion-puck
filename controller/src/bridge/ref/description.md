...
**Role:** Convert binary `struct ble_aircraft_frame` packets coming
off `ble/` into the common `struct aircraft_t`, validate them, and
upsert into `aircraft_db`. Validation lives here, not in `ble/`.
Mirror of the `json/` lib's role for the ADS-B path.
...
shell inject ───┴── bridge_submit_frame() ── k_msgq (8 slots × 38 B) ── bridge_thread
                                                                       │
                                                                       ▼
                                                       bridge_convert() ──▶ aircraft_db_upsert()
```

**Public API:**

- `int  bridge_init(void);` — clears stats; thread is auto-started
  via `K_THREAD_DEFINE`.
- `void bridge_submit_frame(const struct ble_aircraft_frame *);` —
  non-blocking; `k_msgq_put(K_NO_WAIT)`. Safe from any context (the
  BLE stack's BT-rx workqueue calls it).
...
| `ts_ms` (uint32)   | `ts` (double)      | `ts_ms / 1000.0` (sim-node uptime seconds) |
...
`submitted`, `drops` (msgq full), `converted`, `rej_source`,
`rej_icao`, `rej_latlon`.

**Sizing:** 8-slot msgq × 38 B = 304 B. At the sim's 5 Hz cadence
this is 1.6 s of headroom; far more than the consumer thread needs.

**Thread stack:** 4096 B. The original 2048 B sufficed for the
unpack-and-upsert path on its own, but `aircraft_db_upsert()` runs
Kalman matrix math inline (predict + update + closed-form 2×2
inversion) and that pushed the original stack over its guard. The
bump landed during BLE-traffic bring-up.
